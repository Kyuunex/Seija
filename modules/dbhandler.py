	...
	if wheres:
		...
		for onewhere in wheres:
			...
			if onewhere != wheres[-1]:
				wherestring += " AND "
		result = list(c.execute("SELECT %s FROM %s %s;" % (selectkey, tn, wherestring), tuple(wherevalues)))
	else:
...
async def delete(tn, wheres):
	...
	if wheres:
		...
		for onewhere in wheres:
			...
			if onewhere != wheres[-1]:
				wherestring += " AND "
		result = list(c.execute("DELETE FROM %s %s;" % (tn, wherestring), tuple(wherevalues)))
	else:
...
async def insert(tn, values):
	...
	result = list(c.execute("INSERT INTO %s VALUES (%s)" % (tn, formatstring), values))
	conn.commit()
	conn.close()
	return result
