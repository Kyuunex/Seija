...
async def osuprofile(osuprofile):
	if osuprofile:
		...
		osuprofileembed.add_field(
			name="Country", 
			value=':flag_%s: %s' % (osuprofile['country'].lower(), osuprofile['country']), 
			inline=True
			)
		osuprofileembed.add_field(
			name="PP", 
...
async def modpost(subpostobject, beatmapsetdiscussionobject, newevent, trackingtype):
	if subpostobject:
		...
		modpost.set_author(
			...
			url="https://osu.ppy.sh/users/%s" % (str(subpostobject['user_id'])), 
			icon_url="https://a.ppy.sh/%s" % (str(subpostobject['user_id']))
			)
		modpost.set_thumbnail(
			url="https://b.ppy.sh/thumb/%sl.jpg" % (str(beatmapsetdiscussionobject["beatmapset"]["id"]))
			)
		modpost.set_footer(
