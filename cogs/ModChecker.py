...
class ModChecker(commands.Cog, name="Mod Checker"):
            ...
            try:
                ...
                if mapset_id:
                    if str(mapset_id[0][0]) != "0":
                        if await track(str(mapset_id[0][0]), ctx.message.channel.id):
                            try:
    ...
    async def veto(self, ctx, mapset_id: int):
        if db.query(["SELECT value FROM config WHERE setting = ? AND parent = ? AND value = ?", ["guild_veto_channel", str(ctx.guild.id), str(ctx.message.channel.id)]]):
            if await track(mapset_id, ctx.message.channel.id, "veto"):
                try:
                    result = await osu.get_beatmapset(s=mapset_id)
                    embed = await osuembed.beatmapset(result)
    ...
    for user in related_users:
        if str(user_id) == str(user['id']):
            if user['default_group'] == "bng":
                ...
                return user['username']+" [Probation BN]"
            elif user['default_group'] == "nat":
                return user['username']+" [NAT]"
            else:
                return user['username']

async def get_diffname(beatmaps, beatmap_id):
...
async def modpost(subpostobject, beatmapset_discussions, newevent, tracking_mode):
    if subpostobject:
        if tracking_mode == "classic":
            title = str(await get_diffname(beatmapset_discussions["beatmapset"]["beatmaps"], newevent['beatmap_id']))
        elif tracking_mode == "veto":
            title = "%s / %s" % (str(beatmapset_discussions["beatmapset"]["title"]), str(await get_diffname(beatmapset_discussions["beatmapset"]["beatmaps"], newevent['beatmap_id'])))
        ...
        modpost.set_author(
            name=str(await get_username(beatmapset_discussions["beatmapset"]["related_users"], str(subpostobject['user_id']))),
